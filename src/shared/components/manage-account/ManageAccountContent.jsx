      ...
      <div className="flex justify-center space-x-10">
        ...
        <div>
          <div className="bg-black rounded-t-xl text-white py-2 px-4">
            <div className="w-full flex space-x-3 justify-between items-center">
              ...
              <h3 className="flex text-2xl">{isAlpha() ? 'Accounts' : 'Your page'}</h3>
              {isAlpha() ? (
                <span
                  className="ml-10 rounded-full border border-2 border-white px-2 py-1">{Array.isArray(users) ? users.length : 0}</span>
              ) : (
                <div>Toggle</div>)}
            </div>
          </div>
          <div className="border border-[3px] border-black rounded-b-xl space-y-2 p-3">
            ...
            )) : (isAlpha() && Array.isArray(users) && user.length === 0) ? <p>No users found</p> : (
        ...
        <div className="min-w-[300px]">
          <div className="flex justify-between items-center mb-2">
            <h2 className="font-black text-primary text-2xl">Programs.<span
              className="text-black">{schools?.school_list?.length}</span></h2>
